...
class ManageProduct:
    ...
    # Phần 4
    def tinhtongtiennhaphang(self, hang_hoa):
        return hang_hoa.gia_nhap * hang_hoa.so_luong_ton_kho

    def tong_tien_nhap_hang(self):
        if not self.danh_sach_hang_hoa:
            print("Danh sách hàng hoá trống.")
        else:
            print("\nTổng tiền nhập hàng theo sản phẩm:")
            for hang_hoa in self.danh_sach_hang_hoa:
                tiennhaphang = self.tinhtongtiennhaphang(hang_hoa)
                print(
                    f"Mã hàng: {hang_hoa.get_ma_hang()}\t Tên: {hang_hoa.get_ten_hang()}\t "
                    f"Tổng tiền nhập hàng: {tiennhaphang}"
                )
    #phần 8
...
def main():
    ...
    while True:
        ...
        print("7. tổng tiền nhập hàng")
        print("8. tính lại giá mới các hàng gần hết hạn")
        print("9. Thoát")
